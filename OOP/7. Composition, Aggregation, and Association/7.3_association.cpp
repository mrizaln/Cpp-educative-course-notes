// In object-oriented programming, association is the relationship between the unrelated
// objects of the classes. Objects lifespan are not directly tied to each other like in
// composition and aggregation.

// has a relationship of Has-a

// Example
    // The [teacher] has a relationship with his [student]s, but it;s not a part-of relationship
    // as in composition. A teacher can add many students to a class, and a student can be
    // registered in many courses. Neither of the objects ([teacher]  and [student]) lifespan
    // is tied to the other as any student can leave the class of specific teacher and similarly
    // any teacher can change the course which he's currently teaching.
...
private:
    ...
    friend ostream& operator<<(ostream &out, const Teacher &tchr) {
        ...
        cout << tchr.tr_name << " is teaching: ";
        for (int count = 0; count < length; ++count)
            cout << tchr.stdnt[count]->getName() << ' ';
...
ostream& operator<<(ostream &out, const Student &std) {
    ...
    cout << std.Std_name << " is taught by: ";
    for (int count = 0; count < length; ++count)
